...
find_button.addEventListener('click', function () {
  ...
  if (get_check('.vaccined') == true)
    pet_find = pet_find.filter(t => t.vaccine == 'true');

  if (get_check('.dewormed') == true)
    pet_find = pet_find.filter(t => t.deworm == 'true');

  if (get_check('.sterillized') == true)
